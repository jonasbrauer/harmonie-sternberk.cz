    ...
    <div ref="top" v-if="user && user.role === 'admin'">
      ...
      <section class="section pt-2">
        ...
        <transition name="slide-fade" mode="out-in">
          ...
          <span v-if="!newEventForm" class="level is-mobile level-left">
            ...
            <p
              @click="
                eventTemplate = rehearsalTemplate;
                newEventForm = !newEventForm;
              "
              class="button mr-1"
            >
        ...
        <div v-if="events && events.length > 0">
          <div
            ...
            :key="key + 'admineventrow' + event.id"
          >
            <EventLevel
              :event="event"
              @delete="deleteEvent(event.id)"
              @edit="clickEditEvent(event.id)"
            />
            <transition name="slide-fade" mode="out-in">
              <EventForm
      ...
      <section ref="members" class="section pt-2">
        ...
        <transition name="slide-fade" mode="out-in">
          <p
            v-if="!newUserForm"
            v-on:click="newUserForm = !newUserForm"
            class="button"
          >
            <span class="icon mr-1">
              <i class="fa-solid fa-plus"></i> </span
        ...
        <div v-if="users && users.length > 0">
          <div
            class="column"
            v-for="(user, key) in unvalidatedUsers"
            :key="key + 'admin' + user.username"
          >
            <UserLevel
              :user="user"
              @validate="validateUser(user)"
              @delete="deleteUser(user.id)"
              @edit="clickEditUser(user.username)"
            />
        ...
        <div v-if="users && users.length > 0">
          <div
            class="column"
            v-for="(user, key) in validatedUsers"
            :key="key + 'admin' + user.username"
          >
            <UserLevel
              :user="user"
...
export default {
  ...
  methods: {
    postUser(userData) {
      ...
      axios
        .post("/php/user_create.php", userData)
        .then((res) => {
    ...
    postEvent(eventData) {
      ...
      axios
        .post("/php/event_create.php", eventData)
        .then((res) => {
    ...
    deleteEvent(event_id) {
      axios
        .post("/php/event_delete.php", { id: event_id })
    ...
    deleteUser(userId) {
      axios
        .post("/php/user_delete.php", { id: userId })
        .catch((err) => console.error(err))
